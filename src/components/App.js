...
import { UNAUTHORIZED, CONFLICT } from "../utils/constants/errorStatuses";
...
function App() {
  ...
  useEffect(() => {
    ...
    setShortMovieArrayAfterSearch(
      JSON.parse(localStorage.getItem("shortMovieArrayAfterSearch"))
    );
  ...
  const handleChangeShortsCheckbox = (keyWord) => {
  ...
  const searchMovie = (keyWord) => {
    ...
    const movieArrayFromLocalStorage = JSON.parse(
      localStorage.getItem("allMovies")
    );
    const movieArrayAfterSearch = movieArrayFromLocalStorage.filter((film) =>
    ...
    const shortMovieArrayAfterSearch = movieArrayAfterSearch.filter(
      (film) => film.duration <= SHORT_FILM_DURATION
    );
    localStorage.setItem(
      "shortMovieArrayAfterSearch",
      JSON.stringify(shortMovieArrayAfterSearch)
    );
    ...
    if (location.pathname === "/movies" && movieArrayAfterSearch.length === 0) {
      ...
      shortsCheckbox &&
      shortMovieArrayAfterSearch.length === 0
    ) {
      setErrorText("Ничего не найдено");
    }
  };
      ...
      getAllMovies()
        .then((movies) => {
          const filteredMovies = movies.filter((film) =>
            film.trailerLink.startsWith("http")
          );
  ...
  const handleSearchSavedMovies = (keyWord) => {
    ...
    const savedMovieArrayFromLocalStorage = JSON.parse(
      localStorage.getItem("savedMovies")
    );
    const savedMovieArrayAfterSearch = savedMovieArrayFromLocalStorage.filter(
      (film) => film.nameRU.toLowerCase().includes(keyWord.toLowerCase())
    );
    setSavedMovies(savedMovieArrayAfterSearch);
    const shortSavedMovieArrayAfterSearch = savedMovieArrayAfterSearch.filter(
      (film) => film.duration <= SHORT_FILM_DURATION
    );
    setShortSavedMovies(shortSavedMovieArrayAfterSearch);
    if (
      location.pathname === "/saved-movies" &&
      savedMovieArrayAfterSearch.length === 0
    ) {
      setErrorText("Ничего не найдено");
    } else if (
      location.pathname === "/saved-movies" &&
      shortsCheckboxSaved &&
      shortSavedMovieArrayAfterSearch.length === 0
    ) {
      setErrorText("Ничего не найдено");
    }
    setPreloader(false);
  ...
  const getSavedMoviesFromMainApi = () => {
    getSavedMovies()
      .then((savedMovies) => {
        ...
        const shortSavedMovies = savedMovies.filter(
          (film) => film.duration <= SHORT_FILM_DURATION
        );
        localStorage.setItem(
          "shortSavedMovies",
          JSON.stringify(shortSavedMovies)
        );
  ...
  const isMovieAlreadySaved = (movie) => {
    ...
    if (savedMovieArrayFromLocalStorage) {
      return savedMovieArrayFromLocalStorage.some(
  ...
  const getHexId = (movie) => {
    ...
    if (savedMovieArrayFromLocalStorage) {
      return savedMovieArrayFromLocalStorage.find(
        (mov) => mov.movieId === movie.id
      )._id;
    }
  };
      ...
      <CurrentUserContext.Provider value={currentUser}>
        <Routes>
          <Route
            ...
            element={
              <ProtectedRoute loggedIn={!currentUser.loggedIn}>
                <Signup onSignUp={handleSignUp} errorText={errorText} />
              </ProtectedRoute>
            }
          />
          ...
          <Route
            ...
            element={
              <ProtectedRoute loggedIn={!currentUser.loggedIn}>
                <Signin onSignIn={handleSignIn} errorText={errorText} />
              </ProtectedRoute>
            }
          />
